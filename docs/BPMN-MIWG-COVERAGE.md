...
| gelesen, aber leer (kein `isExecutable="true"`) | 11 |
...
| `A.4.0.bpmn` | `rejected` | `parser.NullReferenceException` — `Object reference not set to an instance of an object.` | `rejected` | `bpmn.process.executable_required` an `definitions` | übersprungen (deployment rejected) |
...
| `B.1.0.bpmn` | `rejected` | `parser.NullReferenceException` — `Object reference not set to an instance of an object.` | `rejected` | `bpmn.process.executable_required` an `definitions` | übersprungen (deployment rejected) |
| `B.2.0.bpmn` | `rejected` | `parser.NullReferenceException` — `Object reference not set to an instance of an object.` | `rejected` | `bpmn.process.executable_required` an `definitions` | übersprungen (deployment rejected) |
| `C.1.0.bpmn` | `rejected` | `parser.element.unsupported` — `laneSet` | `rejected` | `bpmn.user_task.form_required` an `userTask` | übersprungen (deployment rejected) |
...
| `C.2.0.bpmn` | `rejected` | `parser.NullReferenceException` — `Object reference not set to an instance of an object.` | `rejected` | `bpmn.process.executable_required` an `definitions` | übersprungen (deployment rejected) |
...
| `C.8.1.bpmn` | `rejected` | `parser.NullReferenceException` — `Object reference not set to an instance of an object.` | `rejected` | `bpmn.element.id_required` | übersprungen (deployment rejected) |
...
| --- | --- | --- | --- |
| `parser.FlowzerModelParseException` | 5 | `C.1.1.bpmn`, `C.3.0.bpmn`, `C.9.0.bpmn`, `C.9.1.bpmn`, `C.9.2.bpmn` | Flowzer verlangt an jedem User-Task ein `zeebe:formDefinition` mit `formKey` oder `formId`. Werkzeugneutrale Modelle tragen keines. **Produktentscheidung, keine BPMN-Lücke.** |
| `parser.NullReferenceException` | 5 | `A.4.0.bpmn`, `B.1.0.bpmn`, `B.2.0.bpmn`, `C.2.0.bpmn`, `C.8.1.bpmn` | `ModelParser.ParseMessages` liest `bpmn:message/@name` als Pflichtattribut (`Attribute("name")!.Value`). BPMN erlaubt den Namen optional; alle fünf Fälle haben ein `bpmn:message` ohne `name` auf Dokumentebene. **Robustheitslücke, keine Fähigkeitslücke.** |
| `parser.element.unsupported` an `laneSet` | 1 | `C.1.0.bpmn` | `ModelParser.GetFlowElements` wirft bei jedem Kindelement eines Prozesses, das es nicht kennt — auch bei reinem Diagramm-Beiwerk ohne Ausführungssemantik. **Lesehürde.** |

> Der Parser wirft beim ersten Kindelement, das er nicht kennt. Gemeldet wird deshalb
> immer nur die **erste** Hürde eines Modells — hinter ihr können weitere liegen.
...
| `bpmn.element.id_required` | `—` | 1 | `C.8.1.bpmn` | `BpmnCapabilityMatrix.IsFlowElement` behandelt jedes nicht ausdrücklich ausgenommene Kindelement eines Prozesses als Flow-Element und verlangt eine `id`. Betroffen ist hier `bpmn:ioSpecification`, ein reguläres BPMN-Element ohne `id`. **Prüfhürde.** |
| `bpmn.element.unsupported` | `businessRuleTask` | 1 | `C.9.0.bpmn` | Elementart, die der Fähigkeitsvertrag v7 nicht führt. **Echte Ausführungslücke.** |
...
Bei 11 davon läuft auch der Parser ohne Ausnahme durch — er liest dann schlicht nichts.
Die übrigen 4 scheitern zusätzlich schon beim Lesen (siehe Gruppe 2).

### 2. Lese- und Prüfhürden vor der eigentlichen Semantik

Diese Fälle scheitern, **bevor** irgendeine Ausführungsfrage gestellt wird:

- **Der Parser lehnt Diagramm-Beiwerk ab.** `ModelParser.GetFlowElements` wirft in seinem
  `default`-Zweig `NotSupportedException` für jedes Kindelement eines Prozesses, das es
  nicht kennt — `laneSet`, `lane`, `textAnnotation`, `association`, `dataObjectReference`
  und Weiteres. Nichts davon trägt Ausführungssemantik. In diesem Satz ist `laneSet` die
  erste solche Hürde; weitere liegen dahinter, weil der Parser beim ersten Treffer abbricht.
- **`bpmn:message` ohne `name` wirft eine `NullReferenceException`.** Der Name ist in BPMN
  optional, `ModelParser.ParseMessages` liest ihn als Pflichtattribut. Eine unbehandelte
  `NullReferenceException` auf fremdem Eingabedokument ist ein Robustheitsmangel, keine
  Fähigkeitsgrenze — sie trifft hier fünf Modelle.
- **`bpmn:ioSpecification` braucht eine `id`.** `BpmnCapabilityMatrix.IsFlowElement`
  behandelt jedes nicht ausdrücklich ausgenommene Kindelement eines Prozesses als
  Flow-Element und verlangt eine `id`. `ioSpecification` ist reguläres BPMN und hat keine.
- **User-Tasks brauchen ein `zeebe:formDefinition`.** Das ist eine bewusste
  ...
  diese Erweiterung nie.

Ein Paket „unbekanntes Beiwerk überlesen statt werfen" plus die beiden Robustheitsfixes
würde die Lesequote deutlich heben, **ohne** die Ausführungszusage zu berühren. Erst danach
zeigt dieser Bericht, welche BPMN-Semantik Flowzer wirklich fehlt.
