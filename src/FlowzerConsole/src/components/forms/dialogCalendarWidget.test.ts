...
import { CALENDAR_LAYER_CLASS, placeCalendar, registerDialogCalendarWidget } from './dialogCalendarWidget';
 ...
 * `appendTo` oder am <body>, `open()` ruft erst die onOpen-Haken und dann die Positionierung.
 */
class FakeFlatpickr {
  ...
  constructor(input: HTMLElement, config: Record<string, unknown>) {
    this.config = { ...config, onOpen: [], onClose: [] };
...
class FakeCalendarWidget {
  ...
  attach(input: HTMLElement) {
    this._input = input;
    // Wie Form.io: Die Lage setzt attach(), nicht der Konstruktor.
...
function createWidget(widgets: WidgetRegistry): TestWidget {
...
/** Ein Startdialog mit einem Datumsfeld in einer Eingabegruppe. */
...
describe('registerDialogCalendarWidget', () => {
  ...
  // Testzweck: Im Dialog hängt der Kalender an einer eigenen Ebene des Dialogelements — innerhalb
  // von role="dialog" (Zeigereingaben, Fokusfalle), aber nicht mehr als statischer Block im
  // Dialoginhalt, der die Bildlauffläche verbreitert (Issue #368).
  it('hängt den Kalender im Dialog an eine eigene Ebene statt statisch ins Formular', async () => {
    const { dialog, input } = dialogWithField();
    const widget = createWidget(registry());

    await widget.attach(input);
    widget.initFlatpickr(FakeFlatpickr);

    const layer = dialog.querySelector(`.${CALENDAR_LAYER_CLASS}`);
  ...
  it('setzt den Kalender beim Öffnen unter das Feld und in den Dialog', async () => {
    ...
    const { dialog, group, input } = dialogWithField();
    const widget = createWidget(registry());
    await widget.attach(input);
    widget.initFlatpickr(FakeFlatpickr);
    ...
    vi.spyOn(layer, 'getBoundingClientRect').mockReturnValue(rect(381, 201, 0, 0));
  ...
  it('folgt dem Feld beim Rollen, solange der Kalender offen ist', async () => {
    ...
    vi.spyOn(layer, 'getBoundingClientRect').mockReturnValue(rect(380, 100, 0, 0));
    const fieldRect = vi.spyOn(group, 'getBoundingClientRect').mockReturnValue(rect(400, 200, 400, 36));
    size(widget.calendar.calendarContainer, 308, 300);

    widget.calendar.open();
    expect(widget.calendar.calendarContainer.style.top).toBe(`${238 - 100}px`);

    fieldRect.mockReturnValue(rect(400, 150, 400, 36));
    group.dispatchEvent(new Event('scroll'));
    expect(widget.calendar.calendarContainer.style.top).toBe(`${188 - 100}px`);

    widget.calendar.close();
    fieldRect.mockReturnValue(rect(400, 100, 400, 36));
    group.dispatchEvent(new Event('scroll'));
    expect(widget.calendar.calendarContainer.style.top).toBe(`${188 - 100}px`);
  });
  ...
  it('räumt die Ebene beim Abbau weg', async () => {
    ...
    expect(dialog.querySelector(`.${CALENDAR_LAYER_CLASS}`)).toBeNull();
  });
});
...
describe('placeCalendar', () => {
  ...
  // Testzweck: Passt der Kalender weder über noch unter das Feld, wird er ans Fenster geklemmt
  // statt oben oder unten aus dem Bild zu ragen.
  it('klemmt den Kalender in niedrigen Fenstern ans Fenster', () => {
    const low = { width: 1280, height: 400 };
    const field = { left: 402, right: 878, top: 150, bottom: 186 };

    const placement = placeCalendar(field, calendar, dialog, low);

    expect(placement.top).toBeGreaterThanOrEqual(8);
    expect(placement.top + calendar.height).toBeLessThanOrEqual(low.height - 8);
  });
});
