...
export function Topbar({ onOpenPalette, onOpenUserMenu }: TopbarProps) {
  ...
  return (
    <header
      className={cn(
        'border-border relative z-[4] flex h-[60px] flex-none items-center gap-4 border-b px-[22px]',
        'backdrop-blur-[10px]',
      )}
      style={{ background: 'color-mix(in oklab, var(--surface) 78%, transparent)' }}
    >
      <nav aria-label="Brotkrumen" className="flex min-w-0 flex-[0_1_auto] items-center gap-[7px] overflow-hidden">
